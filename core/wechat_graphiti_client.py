...
class WeChatGraphitiClient:
    ...
    async def add_episode(self, episode: WeChatEpisode) -> bool:
        """
        添加单个Episode到图数据库
        ...
        try:
            # 转换为Graphiti RawEpisode格式
            raw_episode = self._convert_to_raw_episode(episode)
            ...
            result = await self.graphiti.add_episode(
                ...
                group_id="wechat_relationships",  # 微信关系图谱组
                uuid=raw_episode.uuid,
                update_communities=True
            )
            
            logger.debug(f"成功添加Episode: {episode.episode_id}")
            return True
            
        except Exception as e:
            logger.error(f"添加Episode失败 {episode.episode_id}: {e}")
            return False
    
    async def add_single_episode(self, episode: Dict[str, Any]) -> Dict[str, Any]:
        """
        添加单个Graphiti Episode到图数据库
        
        Args:
            episode: Graphiti Episode字典
        ...
        try:
            ...
            logger.debug(f"成功添加Episode: {episode.get('name', 'Unknown')}")
            return {
                "success": True,
            ...
            error_msg = f"添加Episode失败 {episode.get('name', 'Unknown')}: {e}"
