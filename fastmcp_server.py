    ...
    def _setup_tools(self):
        ...
        @self.mcp.tool(
            ...
            description=(
                    ...
                    "**搜索模式（可选）**：\n"
                    "- 速度模式 (speed=true)：默认使用 embedding 语义搜索，速度快\n"
                    "- 标准模式 (speed=false)：使用 LLM 判断 + embedding 搜索的混合策略，准确性高\n\n"
                    "**调用参数**（字段名区分大小写，必须严格对应）：\n"
                    "- query (字符串，必填)：搜索短语或关键词（中英文均可），用于 Embedding 相似度搜索\n"
                    "- search_results (整数, 可选, 默认=5)：返回的最大搜索结果条数\n"
                    "- speed (布尔值, 可选, 默认=true)：搜索模式开关\n"
                    "  - true：仅 embedding 搜索（快）\n"
                    "  - false：混合搜索（准）\n\n"
                    "**性能建议**：\n"
                    "- 默认使用速度模式\n"
                    "- 需要高准确性时使用标准模式"
        ...
        @self.mcp.tool(
            ...
            description=(
                ...
                "**数据来源**：\n"
                " - 微信聊天记录：人际关系、群组成员、项目协作\n"
                " - 个人记忆写入：通过写入工具添加的关系、偏好、事件\n\n"
                "**特别适用于以下场景**：\n"
                " - 人名查询：例如 \"敏哥\"、\"JZX\"\n"
                " - 项目查询：例如 \"GREEN项目\"、\"Chimera项目开发\"\n"
                " - 关系查询：例如 \"谁参与了GREEN项目\"、\"我和JZX是什么关系\"\n"
                " - 偏好查询：例如 \"我喜欢什么\"、\"咖啡偏好\"\n"
                " - 事件查询：例如 \"我参与了哪些项目\"\n\n"
                "**调用参数**（字段名区分大小写）：\n"
                " - query (字符串，必填)：搜索查询，支持人名、项目名、关系问题、偏好等\n"
                " - max_results (整数，默认3，最大10)：返回的最大结果数量\n\n"
                "**返回格式**：\n"
                " - Top1实体：主要匹配实体 + 评分 + 所有相关关系和实体摘要\n"
                " - Top2-N实体：次要匹配实体 + 评分 + 关系摘要\n"
                " - 自动过滤评分<1.0的无关实体\n"
                " - 关系列表已去重（基于UUID）\n\n"
                "**示例查询**：\n"
                "{\n"
